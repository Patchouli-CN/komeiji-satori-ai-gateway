...
def make_events(state: StateStore, model: str,
                intervals_days: list[float]) -> None:
    ...
    state.append_baseline_event(
        {"ts": ts, "model": model, "event": "refreshed"})
...
class TestTtlLearning:
    def test_cold_start_fixed_30d(self, tmp_path):
        ...
        assert engine.ttl_for("gpt-4o") == (COLD_START_TTL_DAYS, "cold-start", 0)
        make_events(state, "gpt-4o", [10.0, 12.0])  # 只 2 个间隔（3 条事件）
        assert engine.ttl_for("gpt-4o")[0] == COLD_START_TTL_DAYS
    ...
    def test_three_events_factor_07(self, tmp_path):
        ...
        ttl, source, events = engine.ttl_for("gpt-4o")
    ...
    def test_per_model_isolation(self, tmp_path):
        ...
        assert engine.ttl_for("gpt-4o")[0] == pytest.approx(9.0)
        assert engine.ttl_for("claude-x")[0] == COLD_START_TTL_DAYS
...
class TestVerdictAndWarnings:
    ...
    def test_no_collected_at_no_verdict(self, tmp_path):
        ...
        assert engine.verdict("gpt-4o", None) is None

    def test_states_by_consumption(self, tmp_path):
        _, engine = self._engine(tmp_path)
        now = time.time()
        # 手动锁定 10 天 TTL，逐段看状态翻转
        engine.override("gpt-4o", ttl_days=10.0)
        assert engine.verdict("gpt-4o", now - 86400 * 5).state == "fresh"
        assert engine.verdict("gpt-4o", now - 86400 * 8).state == "aging"
        assert engine.verdict("gpt-4o", now - 86400 * 9.5).state == "critical"
        assert engine.verdict("gpt-4o", now - 86400 * 12).state == "expired"

    def test_warning_fires_once_per_state(self, tmp_path):
        _, engine = self._engine(tmp_path)
        now = time.time()
        engine.override("gpt-4o", ttl_days=10.0)
        first = engine.due_warnings(engine.verdict("gpt-4o", now - 86400 * 8))
        assert first and "老化" in first
        # 同一状态不重复点灯
        assert engine.due_warnings(engine.verdict("gpt-4o", now - 86400 * 8.1)) is None
        # 状态升级再点
        second = engine.due_warnings(engine.verdict("gpt-4o", now - 86400 * 12))
    ...
    def test_expired_never_retires_baseline(self, env, tmp_path):
        ...
        verdict = satori.ttl.verdict(st.model, st.collected_at)
...
class TestOverride:
    def test_override_and_persist(self, tmp_path):
        state = StateStore(tmp_path / "s")
        engine = TtlEngine(state)
        engine.override("gpt-4o", ttl_days=45.0, reason="等官方发版")
        # 新引擎（模拟重启）从文件恢复
        engine2 = TtlEngine(state)
        assert engine2.ttl_for("gpt-4o") == (45.0, "override", 0)

    def test_expires_at_auto_unlock(self, tmp_path):
        state = StateStore(tmp_path / "s")
        engine = TtlEngine(state)
        engine.override("gpt-4o", expires_at=time.time() - 10)
        # 已过期的锁定自动解锁，回归冷启动值
        assert engine.ttl_for("gpt-4o")[0] == COLD_START_TTL_DAYS
        assert engine.override_of("gpt-4o") is None
    ...
    def test_endpoint_requires_operator(self, env):
        ...
        assert satori.ttl.override_of("gpt-4o")["reason"] == "等发版"
