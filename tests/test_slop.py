...
from test_phase1 import KEY, build, env
...
class TestSlopScoring:
    ...
    def test_broken_args_scores_and_marks_step(self):
        trace = score_tool_calls(
            self._calls(("get_weather", '{"city":'),  # 断裂的 JSON
                        ), self.DECLARED)
    ...
    def test_unknown_tool_scores(self):
        trace = score_tool_calls(
            self._calls(("hack_the_planet", "{}")), self.DECLARED)
    ...
    def test_no_declared_tools_skips_unknown_check(self):
        ...
        trace = score_tool_calls(
            self._calls(("anything", "{}")), declared_tools=None)
        assert trace.slop_score == 0.0
...
class TestSlopLedger:
    ...
    def test_clean_trace_never_confirms(self):
        ...
        clean = score_tool_calls(
            [{"id": "c", "type": "function",
              "function": {"name": "f", "arguments": "{}"}}], [{"name": "f"}])
...
class TestAnthropicClientAdapter:
    ...
    def test_streaming_tool_events(self):
        ...
        chunk1 = {"choices": [{"delta": {"role": "assistant", "tool_calls": [
...
class TestAnthropicUpstreamPipeline:
    ...
    def test_request_tools_and_tool_messages(self):
        ...
        assert native["tool_choice"] == {"type": "tool",
                                         "name": "get_weather"}
        assistant_msg = native["messages"][1]
        assert assistant_msg["role"] == "assistant"
        tool_use = [b for b in assistant_msg["content"]
    ...
    def test_streaming_tool_use_assembles_canonical(self):
        ...
        chunks += self.conv.translate_sse_data(
            {"type": "message_start", "message": {"id": "m1", "model": "m"}},
            state)
        chunks += self.conv.translate_sse_data(
            {"type": "content_block_start", "index": 0,
             "content_block": {"type": "tool_use", "id": "toolu_1",
                               "name": "get_weather", "input": {}}}, state)
        chunks += self.conv.translate_sse_data(
            {"type": "content_block_delta", "index": 0,
             "delta": {"type": "input_json_delta",
                       "partial_json": '{"city":'}}, state)
        chunks += self.conv.translate_sse_data(
            {"type": "content_block_delta", "index": 0,
             "delta": {"type": "input_json_delta",
                       "partial_json": '"SF"}'}}, state)
        chunks += self.conv.translate_sse_data(
            {"type": "message_delta",
             "delta": {"stop_reason": "tool_use"},
             "usage": {"output_tokens": 5}}, state)

        # 规范侧看到的 tool_calls：index 0、id/name 首片、arguments 分片拼接
        tool_deltas = [c["choices"][0]["delta"]["tool_calls"][0]
                       for c in chunks
    ...
    def test_roundtrip_through_pipeline(self):
        ...
        upstream = Upstream(name="a", base_url="http://x/v1", api_key="k",
                            protocol="anthropic", models=["m"])
...
class TestObserveToolsWiring:
    ...
    def _fwd(self, tools=True):
        ...
        return json.dumps({"tools": [{"type": "function", "function": {
            "name": "get_weather", "parameters": {}}}]}).encode()
    ...
    def test_clean_trace_records_but_never_scores(self, env):
        ...
        asyncio.run(satori._observe_tools(
            "openai", "gpt-4o", self._fwd(),
            [{"id": "c", "type": "function",
              "function": {"name": "get_weather",
                           "arguments": '{"city":"SF"}'}}], "s1"))
    ...
    def test_no_declared_tools_still_scores_broken_args(self, env):
        satori, _ = env
        asyncio.run(satori._observe_tools(
            "openai", "gpt-4o", self._fwd(tools=False), self._broken_calls(),
            "s1"))
