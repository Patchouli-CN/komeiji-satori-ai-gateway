...
satori answers --upstream U --model M                # 采集答案指纹参考作答
...
- **可疑度账本**：所有检测通道的命中都汇入同一本账（按 上游×模型 累计），三级报警等级 `SAFETY`（安全）→ `WATCH`（可能降级，提高关注，默认 25 分）→ `DEGRADED`（质量降级，默认 50 分），跃迁实时广播；账本带半衰期衰减（默认 1 小时）——孤立小错随时间归零，持续掺水照样积聚。另有命中率通道专治"90% 真 10% 假"的掺水战术——用户主动要求角色扮演时负分豁免，不误伤
