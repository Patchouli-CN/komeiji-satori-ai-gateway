...
log = LoggerManager.get_logger("TTL")
...
class TtlEngine:
    ...
    def __init__(
        self, state: StateStore | None = None, overrides_file: Path | None = None
    ) -> None:
        self.state = state
    ...
    def _save_overrides(self) -> None:
        ...
        self.overrides_file.write_text(
            json.dumps(self._overrides, ensure_ascii=False, indent=2), encoding="utf-8"
        )
    ...
    def override(
        ...
        log.info(
            "[ttl] {}/{} 手动锁定保质期：ttl_days={} expires_at={}（{}）",
            upstream,
            model,
            ttl_days,
            expires_at,
    ...
    def override_of(self, upstream: str, model: str) -> dict | None:
        ...
        """
        return self._overrides.get(_okey(upstream, model), self._overrides.get(model))

    def _override_ttl(self, upstream: str, model: str, now: float) -> float | None:
    ...
    def ttl_for(
        self, upstream: str, model: str, now: float | None = None
    ) -> tuple[float, str, int]:
