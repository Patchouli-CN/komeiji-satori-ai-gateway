...
log = LoggerManager.get_logger("STATE")
...
class StateStore:
    ...
    def snapshot(satori) -> dict:
        ...
        return {
            ...
            "identity_ts": {_key(u, m): v for (u, m), v in satori._identity_ts.items()},
            "breakers": {_key(u, m): v for (u, m), v in satori.breakers.items()},
            "breaker_blocks": {
                _key(u, m): v for (u, m), v in satori._breaker_blocks.items()
            },
        }
    ...
    def restore(self, satori, snapshot: dict) -> bool:
        ...
        if not snapshot:
            return False
        satori.suspicion = {
            _split(k): v for k, v in snapshot.get("suspicion", {}).items()
        }
        satori._ledger_ts = {
            _split(k): v for k, v in snapshot.get("ledger_ts", {}).items()
        }
        satori.identity = {
            _split(k): v for k, v in snapshot.get("identity", {}).items()
        }
        satori._identity_ts = {
            _split(k): v for k, v in snapshot.get("identity_ts", {}).items()
        }
        satori.breakers = {
            _split(k): v for k, v in snapshot.get("breakers", {}).items()
        }
        satori._breaker_blocks = {
            _split(k): v for k, v in snapshot.get("breaker_blocks", {}).items()
        }
        log.info(
            "[state] 恢复审计状态：{} 条账本 / {} 个熔断在册",
            len(satori.suspicion),
            len(satori.breakers),
        )
        return True
    ...
    def flush(self, satori) -> None:
        with self._lock:
            ...
            tmp.write_text(
                json.dumps(self.snapshot(satori), ensure_ascii=False), encoding="utf-8"
            )
    ...
    def read_baseline_events(
        self, model: str | None = None, upstream: str | None = None
    ) -> list[dict]:
        events = self._read_jsonl(self.events_file)
        if upstream is not None:
            # 旧格式事件没有 upstream 字段——按通配处理（不丢历史学习原料）
            events = [e for e in events if e.get("upstream", upstream) == upstream]
