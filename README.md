...
- **Suspicion ledger**: every channel's hits flow into one ledger per upstream×model, with three alert levels `SAFETY` → `WATCH` (possible degradation, pay attention, default 25) → `DEGRADED` (quality degraded, default 50), transitions broadcast live — role-play requests from users earn negative-score exemptions, no friendly fire
