...
def _record_refreshed(
    cfg: Config, upstream: str, model: str, pressure_level: str, source: str
) -> None:
    ...
    store.append_baseline_event(
        {
            ...
            "model": model,
            "event": "refreshed",
            "pressure_level_at_collect": pressure_level,
            "source": source,
...
def _clear_retired_meta(cfg: Config, upstream: str, model: str) -> None:
    ...
    if meta.pop("retired_at", None) is not None:
        ...
        print(
            f"[baseline] {upstream}/{model} 已重采——退役标记清除，"
            "判别等级按可信度重新计算"
        )


def _pressure_gate(vendor: str, force_pressure: bool, wait_for_low: bool) -> str:
    ...
    if wait_for_low:
        ...
        print(
            f"[pressure] 等待 LOW/MID 窗口（约 {(target - time.time()) / 60:.0f} 分钟后）…"
        )
    ...
    if level in ("HIGH", "EXTR") and not force_pressure:
        ...
        if answer != "y":
            raise SystemExit(
                "已取消——高压窗口采集的基线会被降权（或用 --force-pressure 强制）"
            )
        level = pressure.level_at()
    return level
...
async def _answers(
    ...
    async with httpx.AsyncClient() as client:
        answers = await collect_answers(
            client, upstream, model, cfg.answerprint.max_tokens
        )
    ...
    sidecar = write_sidecar(
        out, source=source, pressure_level=level, notes=notes, vendor=upstream_name
    )
    _record_refreshed(cfg, upstream_name, model, level, source)
    _clear_retired_meta(cfg, upstream_name, model)
...
def _replay(config_path: str, file: str, tool_traces: str | None = None) -> None:
    ...
    for entry, hits in report.hits[:20]:
        where = (
            f"{entry.get('upstream')}/{entry.get('model')} @{entry.get('ts', 0):.0f}"
        )
        for h in hits:
    ...
    if tool_traces:
        ...
        for rec in tr.chains:
            ...
            head = (
                f"  {rec.get('upstream')}/{rec.get('model')} "
            ...
            for s in rec.get("steps", []):
                ...
                first = (
                    " ↳ 覚「この呼び出し、少し違和感が…」"
                    if rec.get("first_suspicious") == s.get("index")
                    else ""
                )
                print(
...
def _feedback(
    ...
    url = f"http://{cfg.gateway.host}:{cfg.gateway.port}/satori/baseline/feedback"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="satori", description="KomeijiSatori AI gateway"
    )
    ...
    collect.add_argument(
        "--notes", default="", help="来源备注（为什么信它），写进 sidecar"
    )
    collect.add_argument(
        "--force-pressure",
        action="store_true",
    ...
    collect.add_argument(
        "--wait-for-low", action="store_true", help="阻塞至 LOW/MID 窗口再采集"
    )
    ...
    ans.add_argument(
        "--source", default="official", choices=["official", "secondhand", "community"]
    )
    ...
    fb.add_argument(
        "--reason",
        default="false_alarm",
        choices=["official_update", "network_jitter", "false_alarm", "other"],
    )
    ...
    uvicorn.run(
        satori.app, host=cfg.gateway.host, port=cfg.gateway.port, log_config=None
    )
